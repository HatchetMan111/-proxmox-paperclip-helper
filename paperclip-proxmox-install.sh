...
echo "==> [3/6] pnpm installieren (corepack → stabiler Systempfad)..."
# corepack ist in Node.js 20 eingebaut.
# Es installiert pnpm nach /usr/local/bin — den Pfad kennt systemd IMMER.
corepack enable 2>/dev/null || true
corepack prepare pnpm@latest --activate 2>/dev/null || true

# Prüfen ob pnpm jetzt unter /usr/local/bin liegt
if ! command -v pnpm &>/dev/null; then
  echo "    corepack fehlgeschlagen – Fallback auf npm global..."
  npm install -g pnpm@latest
fi

# Absoluten Pfad einmalig fixieren
PNPM_BIN=$(which pnpm)
...
# Store-Verzeichnis setzen
export PNPM_HOME="/root/.local/share/pnpm"
mkdir -p "$PNPM_HOME"
"${PNPM_BIN}" install --frozen-lockfile 2>&1 | tail -8 \
  || "${PNPM_BIN}" install 2>&1 | tail -8
...
# Absoluten pnpm-Pfad nochmal sicherstellen
PNPM_BIN=$(which pnpm)
echo "    systemd ExecStart: ${PNPM_BIN} dev"

# Kurzer Selbsttest
"${PNPM_BIN}" --version > /dev/null \
  && echo "    pnpm Selbsttest: OK" \
  || { echo "FEHLER: pnpm nicht aufrufbar unter ${PNPM_BIN}"; exit 1; }

# systemd Service mit absolutem Pfad
